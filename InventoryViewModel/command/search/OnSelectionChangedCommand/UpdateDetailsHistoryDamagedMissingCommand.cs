...
namespace TEMS.InventoryModel.command.action
{
    ...
    /// </summary>
    public class UpdateDetailsHistoryDamagedMissingCommand : OnSelectionChangedCommand
    {
        public UpdateDetailsHistoryDamagedMissingCommand(DetailsViewModelBase detailsPaneVM) : base(detailsPaneVM)
        {
        }
        ...
        protected override void UpdateDetailsPane(SearchResult selectedItem)
        {
            ...
            if (detailsPaneVM is HistoryDamagedMissingViewModel viewModel)
            {
                viewModel.EventList = new ObservableCollection<ItemBase>(((IEnumerable)DataRepository.GetDataRepository.GetDamageMissingEvents(selectedItem)).Cast<ItemBase>());
            }
        }
    }
}